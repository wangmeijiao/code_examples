...
##iteLift tested and improved but still experimental at 10.28, 2016
##stop when multiblast links


my($tab,$region,$gap,$step);
GetOptions("tab=s",\$tab,"region=s",\$region,"gapSize=i",\$gap,"iterStep=i",\$step);
$gap||=50000;
$step||=10;
...
sub iteLift(){
  ##iterally call liftOver by step, if has multiple blast hits or no hits etc.
  my ($region,$tab_idx,$gap,$step)= @_;
  $step||=1;
  ...
  while(1){   #add #nohit, flip result start end
    ...
    if($region_lifted=~/^##/){
       ...
       if($region_lifted=~/^##start/){  ##start side has no mapping result, cut step bp and research
         ...
         #$start_coord+=$step;
       }elsif($region_lifted=~/^##end/){ ##end side has no mapping result, cut step bp and research
           if($start_coord - $box[1] == 0 && $end_coord - $box[2] != 0){
                $end_coord-=$step;
               }elsif($start_coord - $box[1] != 0 && $end_coord - $box[2] == 0){
                  $start_coord+=$step;
           ...
           #$end_coord-=$step;
          }elsif($region_lifted=~/^##gap too large/){ #start and end mapped onto two different segments, try cut and research
              if($start_coord - $box[1] == 0 && $end_coord - $box[2] != 0){ 
                $end_coord-=$step;
               }elsif($start_coord - $box[1] != 0 && $end_coord - $box[2] == 0){
                  $start_coord+=$step;
            ...
            }elsif($region_lifted=~/^##one end has multiple/){  # stop and you should filter blast links to reduce complexity or try cut step bp and research
                 ...
                 if($start_coord - $box[1] == 0 && $end_coord - $box[2] != 0){ 
                    $end_coord-=$step
                      ...
                      $start_coord+=$step
                        ...
                        #$end_coord-=$step;$start_coord+=$step
                        die"$region_lifted, $start_coord - $box[1] == 0 && $end_coord - $box[2] == 0";
                        #$result="multihit at both end";last
                      }else{die "err: $region_lifted,  $start_coord - $end_coord | $box[1] - $box[2]"}
#=cut                  
...
sub liftOver(){
 ...
 #1, map start coordinate and end coordinate seperately
   my $cnt_blast_link_start;
   my $cnt_blast_link_end;
   foreach my$ctrl(keys %{$tab}){
      ...
      if($start >= $qstart && $start <= $qend){  #qstart always < qend
         $cnt_blast_link_start++;
         #get query gaps
         print STDERR "\nliftOver:start($start):query, blast link ($cnt_blast_link_start) is :$qstart,$qend => $chr_t,$tstart,$tend \n\n";
         my $qgap_count = &gapQuery($start,$qgap,0);
         my $alignLen_map = abs($start-$qstart)+1+$qgap_count; 
      ...
      if($end >= $qstart && $end <= $qend){  #qstart always < qend
         $cnt_blast_link_end++;
         #get query gaps
         print STDERR "\nliftOver:end($end):query,  blast link ($cnt_blast_link_end) is :$qstart,$qend => $chr_t,$tstart,$tend \n\n";
         my $qgap_count = &gapQuery($end,$qgap,0);
         my $alignLen_map = abs($end-$qstart)+1+$qgap_count; 
