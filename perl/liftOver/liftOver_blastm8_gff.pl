...
#usage: perl liftOver_blastm8_gff.pl -tab H1_japo-VS-H1_zs97.blast -ingff japo.mRNA_CDS.embl.gff -gapSize 50000 -iterStep 100 > japo.mRNA_CDS.embl.liftOver.H1_zs97.gff 2> liftOver.err 

#this script is based on liftOver_mummer.pl
#version 1.0 finished at 2016, 9.18 
#iteLift need test and improvement

## correct minus strand mapping error, Oct. 28, 2016

my($tab,$ingff,$gap,$step);
GetOptions("tab=s",\$tab,"ingff=s",\$ingff,"gapSize=i",\$gap,"iterStep=i",\$step);
$gap||=50000;
$step||=10;
...
##test region
#my $region_lifted = &liftOver("375310-375422",$tab_idx->{"H1_japo"},$gap); 
#my $region_lifted = &iteLift("H1_japo:375310-375422",$tab_idx,$step,$gap);
#if($region_lifted ne "nohit"){print "H1_japo:375310-375422 -> $region_lifted\n"}else{print STDERR "liftOver fail for region H1_japo:375310-375422\n"} 

##try to keep gff structure and lift at the same time
#my ($gff,$mrna) = &gffRead_mRNA($ingff);
#print Dumper $gff,$mrna; exit;
#foreach my $chr(sort keys %{$gff}){
# foreach my $id(%{$gff->{$chr}}){
#     foreach my $feat(%{$gff->{$chr}->{$id}}){

#     }   
# }#foreach id end
#}#foreach chr end

##simple liftOver regions, gff and bed 
open GFF, $ingff or die "$!";
my $cnt;
my $cnt_ok;
while(<GFF>){
  chomp;
  next if($_ eq "" || $_=~/^#/ || $_=~/^\s+$/);
  my ($chr,$source,$ftype,$start,$end,$score,$strand,$fshift,$attr)=split/\t/,$_;
  ...
  print STDERR "###begin to lift $ftype region $chr:$start-$end..###\n";
  ...
  if($region_lifted !~/^#/ ){
    ...
    print "$chr_t\tliftOver_blast\t$ftype\t$s\t$e\t$score\t$strand\t$fshift\t$attr\n";
    print STDERR "$ftype $chr:$start-$end liftOver sucessfully to $region_lifted\n";
    $cnt_ok++
  }else{print "#$chr\t$source\t$ftype\t$start\t$end\t$score\t$strand\t$fshift\t$attr\n";print STDERR "$ftype $chr:$start-$end liftOver fail: $region_lifted\n"}
  ...
  print STDERR "###Finished to lift $ftype region $chr:$start-$end###\n\n";
  $cnt++;
...
while(<M8>){
  ...
  #die "qseq != alignlen || sseq != alignlen at $_" if(length $qseq != $alignLen-1 || length $sseq != $alignLen-1);
 ...
 #1, map start coordinate and end coordinate seperately
   foreach my$ctrl(keys %{$tab}){
      ...
      if($end >= $qstart && $end <= $qend){  #qstart always < qend
         #get query gaps
         print STDERR "liftOver:end($end):query\n";
