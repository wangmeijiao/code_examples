...
#######version#########
# March 16, 2017 change sub_family to sub_family.family  
#Sep.7 2016 tolerate start == end (with strong warning) while keep die when start > end
...
while(<stdin>){
  ...
  print STDERR "warning!!! check start == end at line @box\n" if($qs==$qe || $ts==$te);
...
foreach my $id(sort{$a<=>$b} keys %te){
   ...
   if(scalar @group >1){ 
     ...
     if(!defined $family || $family eq""){die "\t$id:can't identify te class at @firstLine\n"}elsif(!defined $subfamily || $subfamily eq ""){$subfamily="unamed_subfamily"."_".$family;$subsubfamily="unamed_subsub"}elsif(!defined $subsubfamily || $subsubfamily eq ""){$subsubfamily="unamed_subsub"}
