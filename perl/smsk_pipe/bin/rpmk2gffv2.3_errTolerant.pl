...
while(<stdin>){
  ...
  $_=~s/^ +| +$//;
  my @box=split/ +/,$_;
  if(scalar @box !=16 && scalar @box != 15){print STDERR "column number err at $_\n";next};
  ##rmsk_out format:
  ...
  print STDERR "check start>=end at line @box\n" if($qs>=$qe || $ts>=$te);
             ...
             if(defined $bed){
                ...
                print OUT "$temp[0]\t$temp[3]\t$temp[4]\tte$target|$detail\t.\t$temp[6]","\n";
                      ...
                      foreach(0..$#{$index}-1){
                         ...
                         print STDERR "ts == te at $index->[$_]" if($ts == $te);
                         my $total_len=$te+$tleft;
