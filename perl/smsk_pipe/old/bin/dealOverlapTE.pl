        ...
        for (my $j=0;$j<$n;$j++) {
                ...
                $E1=$array_order[$j][1];
                    ...
                    if (defined $array_order[$j+1][0]){
                      ...
                      print "$array[-1][0]\t$array[-1][1] : $S1\t$E1 : $S2\t$E2\n";
                      #1,first-mid-second insulated from each other
                      if ( ($E1<$S2) && ($S1>$array[-1][1]) ) {
                          push @array,[$S1,$E1,$array_order[$j][2],$array_order[$j][3]];
                       #2,first overlap with mid
                       }elsif ( ($E1<$S2) && ($S1<=$array[-1][1]) ) {
                              $array[-1][1]=$E1;
                              if ($E1-$S1 > $array[-1][1]-$array[-1][0]){ #update short first segment anno to current longer
                                $array[-1][2]=$array_order[$j][2];
                                $array[-1][3]=$array_order[$j][3];
                              }
                         #3,mid overlap with second
                         }elsif ( ($E1>=$S2) && ($S1>$array[-1][1]) ) {
                               push @array,[$S1,$E1,$array_order[$j][2],$array_order[$j][3]];
                           #4,first overlap with mid and mid overlap with second
                           }elsif ( ($E1>=$S2) && ($S1<=$array[-1][1]) ) {
                             $array[-1][1]=$E2; 
                             # mid longer than first and second
                             if ($E1-$S1 > $array[-1][1]-$array[-1][0] and $E1-$S1 > $E2-$S2){
                                $array[-1][2]=$array_order[$j][2];
                                $array[-1][3]=$array_order[$j][3];
                              # second longer than first and mid
                          ...
                          #print "not defined:$array_order[$j][0]\n";
                          if ( $S1>$array[-1][1] ) {
                             push @array,[$S1,$E1,$array_order[$j][2],$array_order[$j][3]];
