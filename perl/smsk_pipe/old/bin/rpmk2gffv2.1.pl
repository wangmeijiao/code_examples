...
foreach my $id(sort{$a<=>$b} keys %te){
   ...
   if(scalar @group >1){ 
     #check: all fragments must have the same strand, class or qID, target, output to stderr if not
     my $flag_break=0;
     for(0..$#group-1){
        my @box1=split/:/,$group[$_];
        my @box2=split/:/,$group[$_+1];
        if($box1[4] ne $box2[4] || $box1[8] ne $box2[8] || $box1[9] ne $box2[9] || $box1[10] ne $box2[10]){
           print STDERR "qID, strand or target, class not the same in a group at group $id(output to lost.gff/bed): \n  @box1\n  @box2\n"; 
           $flag_break=1;
        }    
     }
     ...
     my $attribs="ID=".$id."#".$firstLine[9].";"."Note="."$firstLine[9]#$firstLine[10]|coverage_total:$coverage_total%|identity_ave:$identity_ave%";
     ...
     if(!defined $family || $family eq""){die "\t$id:can't identify te class at @firstLine\n"}elsif(!defined $subfamily || $subfamily eq ""){$subfamily="unamed_subfamily";$subsubfamily="unamed_subsub"}elsif(!defined $subsubfamily || $subsubfamily eq ""){$subsubfamily="unamed_subsub"}
      $class{$family}->{$subfamily}->{$subsubfamily}->{$id}=\@group_sorted;
        ...
        my $coverage=sprintf "%.3f",100*($te-$ts+1)/($te+$tleft);
        if(defined $filter){next if($coverage< $filter*100)};
        my $attribs="ID=".$id."#".$tID.";"."Note="."$tID#$class|coverage:$coverage%|identity:$identity%";
             ...
             if(defined $bed){
                ...
                print OUT "$temp[0]\t$temp[3]\t$temp[4]\tte$target|$detail\t.\t$temp[6]","\n";
                      ...
                      foreach(0..$#{$index}-1){
                         ...
                         print OUT "$qID\trmsk2gffv2\trepeat_fragment\t$qs\t$qe\t.\t$strand\t.\tParent=$id#$tID\n";
                      }
                    }
