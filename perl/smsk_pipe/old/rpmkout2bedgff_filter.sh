...
if [ $type = "bed" ]; then
  echo "do $type"
  outdir=beds_$prefix
  cat out_${prefix}-smsk/${prefix}.fa.out |perl bin/rpmk2gffv2.1.pl -bed -prefix $prefix -filter 0.3 -outdir $outdir 2> out2bed.err
  ...
  echo "do $type"
  outdir=gffs_$prefix
  cat out_${prefix}-smsk/${prefix}.fa.out |perl bin/rpmk2gffv2.1.pl  -prefix $prefix -filter 0.3 -outdir $outdir 2> out2gff.err
