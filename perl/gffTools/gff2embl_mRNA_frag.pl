...
my ($gff_idx,$mrna_idx)=&gffRead_mRNA_frag($ARGV[0]);

#print Dumper $gff_idx,$mrna_idx; exit;
...
#  foreach my $geneID(sort keys %{$gff_idx->{$chr}}){
     foreach my $mrnaID(keys %{$gff_idx->{$chr}}){
         ...
         my ($chr_mrna,$start_mrna,$end_mrna,$ID_mrna,$strand_mrna,$note);
         if(exists $mrna_idx->{$mrnaID}){($chr_mrna,$start_mrna,$end_mrna,$ID_mrna,$strand_mrna,$note)=split/\t/,$mrna_idx->{$mrnaID}}else{  
              $ID_mrna = $mrnaID;
              $note = $mrnaID;
              (undef,$strand_mrna) = split/:/,$gff_idx->{$chr}->{$mrnaID}->{"exon"}->[0];#print STDERR "$ID_mrna\t$strand_mrna\n";
            }
         ...
         if(exists $element->{"exon"}){
            ...
            printf("%-23s%s\n","FT","/gene=\"$ID_mrna\"");         
            printf("%-23s%s\n","FT","/note=\"$note\"");         
         }else{die"no exon in mrna $mrnaID"}
     ...
     #}elsif($ftype eq "mRNA"){
      ...
      #  }else{die "dup mRNA ID at $ID\n"}
           ...
           if(!exists $gff{$chr}->{$pID}->{$ftype}){
              my @temp;
              if($ftype eq "CDS"){push @temp,"$start-$end-$fshift:$strand"}else{push @temp,"$start-$end:$strand"};
              $gff{$chr}->{$pID}->{$ftype}=\@temp;
           }else{ if($ftype eq "CDS"){push @{$gff{$chr}->{$pID}->{$ftype}},"$start-$end-$fshift:$strand"}else{push  @{$gff{$chr}->{$pID}->{$ftype}},"$start-$end:$strand"} }
