...
###inherited from gffSimplifyv1.3
#read in gtf and processed then output gff standard processed 
#Jun 6, 2017
...
my ($cutoff,$exon,$intron,$tss,$tts);
GetOptions("cutoff=i",\$cutoff,"exon!",\$exon,"intron!",\$intron,"tss!",\$tss,"tts!",\$tts);
...
foreach my$chr(sort keys %gtf){
 ...
 foreach my $gene(sort keys %{$gtf{$chr}}){
   my ($chr,$start,$end,$id,$strand,$note)=split/\t/,$gene{$gene};
   if($end-$start+1<$cutoff){print SHORT "$chr\tgff_simplifyv1.3\tgene\t$start\t$end\t.\t$strand\t.\tID=$id;Name=$note\n";next}
   #if($note!~/retrotransposon|transposon/){
   if($note!~/retrotransposon|transposon|rRNA|tRNA|miRNA|ncRNA|snRNA|snoRNA|rRNA|other_RNA|mRNA_TE_gene|pseudogene|transposable_element_gene|mitochondrion|chloroplast/){
    print "$chr\tgtf_simplifyv1.3\tgene\t$start\t$end\t.\t$strand\t.\tID=$id;Name=$note\n";
    my $max_len=0;
    my $max_len_mrna;
    my @groups;
    foreach my $mrna(keys %{$gtf{$chr}->{$gene}}){
    ...
    my $index=$gtf{$chr}->{$gene}->{$max_len_mrna};
    my ($chr_mrna,$start_mrna,$end_mrna,$id_mrna,$strand_mrna,$pID,$name)=split/\t/,$mrna{$max_len_mrna};    
    print "$chr_mrna\tgtf_simplifyv1.3\tmRNA\t$start_mrna\t$end_mrna\t.\t$strand_mrna\t.\tID=$id_mrna;Parent=$pID;Note=longest_AS;Group=$group;\n";
    my $alias_mrna = $id_mrna;
    ...
    if(exists $index->{"CDS"}){
       ...
       foreach(@CDS){
         ...
         print "$chr_mrna\tgtf_simplifyv1.3\tCDS\t$start\t$end\t.\t$strand_mrna\t$fshift\tParent=$alias_mrna;Note=CDS$i\n";
         push @CDS_store,"$start-$end";
       } 
    }else{print STDERR "warnings!! no CDS at $id note:$note\n"}
    #}else{die "no CDS at $id note:$note\n"} #CDS is a must

    #UTRs (utrs are start or end exons)
    my @utr5;# for exon and intron calculation
    if(exists $index->{"5UTR"}){
       my @box=@{&segSort_strand($index->{"5UTR"},$strand_mrna)};
       my $i;
       foreach(@box){
         $i++;
         my($start,$end)=split/-/,$_;
         print "$chr_mrna\tgtf_simplifyv1.3\tfive_prime_UTR\t$start\t$end\t.\t$strand_mrna\t.\tParent=$alias_mrna;Note=utr5_$i\n";
         push @utr5,"$start-$end";
       }
    }
    my @utr3;# for exon and intron calculation
    if(exists $index->{"3UTR"}){
       my @box=@{&segSort_strand($index->{"3UTR"},$strand_mrna)};
       my $i;
       foreach(@box){
         $i++;
         my($start,$end)=split/-/,$_;
         print "$chr_mrna\tgtf_simplifyv1.3\tthree_prime_UTR\t$start\t$end\t.\t$strand_mrna\t.\tParent=$alias_mrna;Note=utr3_$i\n";
         push @utr3,"$start-$end";
       }
    }
    ...
    if(exists $index->{"exon"}){
       ...
       foreach(@exon){
         $i++;
         my($start,$end)=split/-/,$_;
         print "$chr_mrna\tgtf_simplifyv1.3\texon\t$start\t$end\t.\t$strand_mrna\t.\tParent=$alias_mrna;Note=exon$i\n";
       } 
    }
    ...
    if(exists $index->{"intron"}){
       ...
       foreach(@intron){
         $i++;
         my($start,$end)=split/-/,$_;
         print "$chr_mrna\tgtf_simplifyv1.3\tintron\t$start\t$end\t.\t$strand_mrna\t.\tParent=$alias_mrna;Note=intron$i\n";
       } 
    }
    ...
    }#calculate exon and/or intron end

   
    #calculate intron from already exist exons
    if(!exists $index->{"intron"} && exists $index->{"exon"} && $intron){
       ...
       if(scalar @{$exon} >1){
         ...
         foreach(@$intron){
             my($start,$end)=split/-/,$_;
             $i++;
             print "$chr_mrna\tgtf_simplifyv1.3\tintron\t$start\t$end\t.\t$strand_mrna\t.\tParent=$alias_mrna;Note=calculated_intron$i\n";
         }
       }elsif(scalar @{$exon} == 1){
              print STDERR "$alias_mrna has one exon :@$exon and can't calculate intron\n"
           }
    }#calculate intron end



  }else{
         #TE related gff, output as is; useful for DNA TE detail annotation, i.e CACTA transpoase 
         print TE_G "$chr\tgff_simplify\tgene\t$start\t$end\t.\t$strand\t.\tID=$id;Name=$note\n";
         foreach my $mrna(sort keys %{$gtf{$chr}->{$gene}}){
           my $index=$gtf{$chr}->{$gene}->{$mrna};
           my ($chr_mrna,$start_mrna,$end_mrna,$id_mrna,$strand_mrna,$pID)=split/\t/,$mrna{$mrna};
           my $alias_mrna = $id_mrna;
           print TE_G "$chr_mrna\tgff_simplifyv1.3\tmRNA\t$start_mrna\t$end_mrna\t.\t$strand_mrna\t.\tID=$alias_mrna;Parent=$id\n";
