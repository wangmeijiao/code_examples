...
while(<stdin>){
  ...
  if(&checkRealSeq($seq_index) == 0){print STDERR "real seq is empty ";print STDERR Dumper $seq_index;print STDERR "\n";next}
  ...
  if(!$quiet){ ## detail mode
    foreach my $id(sort keys %align){printf("%-40s  %s\n",$id,$align{$id})}
    printf "%-40s  %s\n"," ",join("",@{$stars});
    printf "%-40s  %s\n"," ",join("",@{$densi_score});
    printf "%-40s  %s\n"," ",join("",@{$cons_seq});
    printf "%-40s  %s\n"," ",join("",@{$flag_gaps});
