...
# header=T/F
# check and make sure that not have NA and empty values, all datapoint must be numeric (except the first column)
...
foreach (@{$data_idx}){
 ...
 my @box=split/[\t ]+/,$_;
 my $id=shift @box;
 $svg->text("x",$x_left-15,"y",$y+4,"width",$box_w,"height",10,"font-family","Arial", "font-weight","bold","text-anchor","end","font-size",8, "-cdata", $id);
 my $x=0;
 foreach my $data(@box){   
   my $colN; #color index
...
foreach(@{$header_idx}){
   $svg->text("x",$text_x,"y",$text_y,"width",$box_w,"height",10,"font-family","Arial", "font-weight","bold","text-anchor","end","font-size",10, "-cdata", $_);
   $text_x+=$box_w+10;  
}
...
sub drawColorBar(){
   ...
   if($type eq "v"){
      foreach(reverse @color){
        $svg->rect("x",$x,"y",$y,"height",$ppx_local,"width",15,"fill",$_,"stroke",$_);#stroke "none" unwork
        $y+=$ppx_local;
      }
    }elsif($type eq "h"){
        foreach(@color){
    ...
    #draw text
    my($x_text,$y_text)=($x+25,$y-0.18*$len);
    $svg->text("x",$x_text,"y",$y_text,"-cdata","heatmap density(0 to max)","transform","rotate(-90,$x_text,$y_text)","stroke","black");
   return 1;
}
