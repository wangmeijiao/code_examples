#!/bin/bash
##########################################################
##     data2bw mapping pipeline for ChIP-seq fqpe       ##
##     Sep 11, 2013                                     ## 
##########################################################
...
#1. mapping by bowtie
 echo "start bowtie..(multi)"
 bowtie --al uniqual.fq --un unmapped.fq --max multi.fq -v 0 -M 1 -X 500 --best --strata -y --sam --threads 14 $bowtieidx -1 ${data}_1.fq -2 ${data}_2.fq hits_${prefix}.sam  2>bowtie.err
